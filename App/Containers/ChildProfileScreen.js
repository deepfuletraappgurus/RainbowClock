...
export default class ChildProfileScreen extends BaseComponent {
  ...
  isValidate = () => {
    if (this.state.objSelectedChild.name.trim() == '') {
      ...
      !Helper.validateChildName(this.state.objSelectedChild.name.trim())
    ) {
      Helper.showErrorMessage(Constants.MESSAGE_VALID_CHILD_NAME);
      return false;
    } else if (
      JSON.stringify(this.state.dupObjSelectedChild) ===
      JSON.stringify(this.state.objSelectedChild)
    ) {
  ...
  renderRow(item, index) {
    return (
      <TouchableOpacity
        style={[
          styles.rewardTouch,
          this.state.selectedRewardIcon == item ? styles.rewardTouchActive : '',
        ]}
    ...
    const res = objSecureAPI
      ...
      .then(resJSON => {
        if (resJSON.ok && resJSON.status == 200) {
          ...
          if (resJSON.data.success) {
            this.setState({
              dupObjSelectedChild: JSON.stringify(resJSON.data.data[0]),
            });
            try {
                ...
                <View style={styles.imageUploader}>
                  ...
                  <TouchableOpacity
                    ...
                    <Text style={styles.smallButtonText}>
                      {'Change Profile Image'.toUpperCase()}
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
