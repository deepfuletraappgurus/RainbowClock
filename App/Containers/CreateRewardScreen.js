...
export default class CreateRewardScreen extends BaseComponent {
    ...
    isValidate = () => {
        if (this.state.rewardImage == '') {
            ...
            return false;
        } else if (this.state.numberOfToken.trim().toLocaleLowerCase() === 'select' || this.state.numberOfToken.trim() == '') {
    ...
    renderTokens(item, index) {
        return (
            <TouchableOpacity style={styles.dropdownItem} onPress={() => this.setState({ numberOfToken: item, showDropdown: false })}>
                ...
                <ImageBackground source={Images.blueBackground} style={styles.backgroundImage}>
                    <ScrollView contentContainerStyle={[styles.ScrollView]} keyboardShouldPersistTaps='never'>
                        <View style={styles.container}>
                            <Text style={[styles.h1, styles.textCenter, {marginBottom:10}]}>{'Create Reward'.toUpperCase()}</Text>
                            <View style={styles.form}>
                                <View style={styles.imageUploader}>
                                    <View style={styles.uploadView}>
                                        <Image source={this.state.rewardImage ? { uri: this.state.rewardImage.path } : Images.upload} style={this.state.rewardImage ? styles.uploadedImage : styles.uploadPlaceholder} />
                                    </View>
                                    <View style={styles.buttonRight}>
                                        <TouchableOpacity style={[styles.button, styles.smallButton, styles.buttonCarrot, { marginBottom: 0 }]}
                                            ...
                                            <Text style={styles.smallButtonText}>{'Select Image'.toUpperCase()}</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                                ...
                                <View style={[styles.iconContainer, { flexGrow: 1 }]}>
                                    <Text style={[styles.title, styles.textCenter]}>{'Type of Tokens'.toUpperCase()}</Text>
                                    <View style={styles.typeTokens}>
                                        <TouchableOpacity
                                            style={styles.tokensClick}
                                            onPress={() => this.chooseToken(false)}>
                                            <View style={this.state.isSpecialReward ? styles.disable : null}>
                                                <View style={styles.tokensIconView}>
                                                    <Image source={Images.everyday} style={styles.tokensIcon} />
                                                </View>
                                                <Text style={[styles.tokensText, styles.textCenter]}>{'EVERYDAY\nREWARD'.toUpperCase()}</Text>
                                            </View>
                                        </TouchableOpacity>
                                        <TouchableOpacity
                                            style={styles.tokensClick}
                                            onPress={() => this.chooseToken(true)}>
                                            <View style={this.state.isSpecialReward ? null : styles.disable}>
                                                <View style={styles.tokensIconView}>
                                                    <Image source={Images.special} style={styles.tokensIcon} />
                                                </View>
                                                <Text style={[styles.tokensText, styles.textCenter]}>{'SPECIAL\nREWARD'.toUpperCase()}</Text>
                                            </View>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                                ...
                                <View style={[styles.formControl, styles.formControlSmall]}>
                                    <TextInput
                                        ...
                                        underlineColorAndroid={'transparent'}
                                        placeholderTextColor={'#fff'}
                                        onChangeText={(numberOfToken) => this.setState({ numberOfToken })}
                                        keyboardType='number-pad'
                                    />
                                </View>
                            </View>
                            <View style={{ paddingLeft: 30, paddingRight: 30, width: '100%' }}>
                                <TouchableOpacity style={[styles.button, styles.buttonPrimary, { marginBottom: 0 }]} onPress={() => this.onPressCreate()}>
                                    {this.state.loading ?
                                        <Spinner size='small' color={Colors.snow} />
                                        :
                                        <Text style={styles.buttonText}>{'Create'.toUpperCase()}</Text>
                                    }
                                </TouchableOpacity>
                            </View>
