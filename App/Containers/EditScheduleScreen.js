...
export default class EditScheduleScreen extends BaseComponent {
    ...
    componentDidMount(){
        ...
        this.setState({
            ...
            const scheduleDetails = this.props.navigation.getParam('scheduleDetails', {});
            if(scheduleDetails){
                console.log('scheduleDetails',scheduleDetails)
                const taskName = this.state.taskNameList.includes(scheduleDetails?.name);
                this.setState({ selectedTask: taskName ? scheduleDetails?.name : '+CREATE YOUR OWN' ,taskName : !taskName ? scheduleDetails?.name : '' })
                this.setState({
    ...
    moveToScheduleTask = () => {
        ...
        if (this.isValidate(task_dates)) {
            ...
            this.props.navigation.navigate('EditSelectTaskScreen', { dictCreateTask: dictCreateTask })
        }
    }
    ...
    showTimePicker() {
       ...
       onTimeSelected(event, value) {
        ...
        this.state.time = Helper.dateFormater(new Date(value), "hh:mm A", 'hh:mm A')
        this.state.fromTimeFormate = Helper.dateFormater(new Date(value), "hh:mm A", 'A')
      ...
      showToTimePicker() {
       ...
       onToTimeSelected(event, value) {
        ...
        this.state.toTimeDate = Helper.dateFormater(new Date(value), "hh:mm A", 'hh:mm A')
        this.state.toTimeFormate = Helper.dateFormater(new Date(value), "hh:mm A", 'A')
        //this.changeFromDate(value)
                        ...
                        <ScrollView contentContainerStyle={styles.ScrollView}>
                            <View style={[styles.container, styles.justifySpaceBetween]}>
                                <View style={styles.topHeader}>
                                    <Text style={[styles.h1, styles.textCenter]}>{'EDIT SCHEDULE'.toUpperCase()}</Text>
                                </View>
                                            ...
                                            <TouchableOpacity onPress={()=> {
                                               ...
                                               <DatePicker
                                                ...
                                                />)
                                                    ...
                                                    {/* <TouchableOpacity style={[styles.inputField, { paddingHorizontal: 0, overflow: 'hidden' }]}
                                                        // <TouchableOpacity style={[styles.inputView, {paddingHorizontal:0, overflow:'hidden'}]}
                                                        onPress={() => Helper.showErrorMessage('Please Select From time first')}>
                                                        {this.state.fromTime.length == 0 ?
                                                            <View style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, backgroundColor: Colors.transparent, alignItems: 'center', justifyContent: 'center' }}>
                                                                <Text style={{ color: '#c9c9c9' }}>HH:MM</Text>
                                                            </View>
                                                            :
                                                            // <DatePicker full transparent style={[styles.rowSectionWhite,{width: '100%'}]}
                                                            //     disabled={this.state.fromTime.length == 0 ? true : false}
                                                            //     // style={{ width: '100%' }}
                                                            //     placeholder='HH:MM'
                                                            ...
                                                            //     confirmBtnText="Confirm"
                                                            //     cancelBtnText="Cancel"
                                                            //     minDate={Helper.getMinimumTime(this.state.fromTime)}
                                                            //     showIcon={false}
                                                            //     onDateChange={(toTime) => { this.setState({ toTime }) }}
                                                            //     customStyles={{
                                                            //         dateInput: {
                                                            //             marginLeft: 0,
                                                            //             borderWidth: 0,
